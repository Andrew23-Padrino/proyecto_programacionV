...
import AstronomiaLesson from './AstronomiaLesson.js'
import { loadOriginalById } from '../loader.js'
import AstronomiaCanvasGame from './AstronomiaCanvasGame.js'
  ...
  (async ()=>{
    try{
      ...
      const startGame = async (opts) => {
        const container = document.getElementById('astro-container') || document.body;
        ...
        new AstronomiaCanvasGame('astro-canvas', { lessonScore, onWin: async ({ recorder: rec })=>{ await onFinish({ winner:true, points:10, subject:'astronomia', lessonScore, recorder: rec }) } })
      };
      new AstronomiaLesson(startGame);
    }catch(e){ showLoginRequiredModal(); }
  })();
});

async function onFinish({ winner = false, points = 0, subject = 'astronomia', lessonScore = 0, recorder }){
  try{
...
class AstronomiaGame extends Game {
  constructor(opts = {}) { super({ id: 'astronomia', name: 'Astronomía', assets: opts.assets }) }
  async init(container) { await super.init(container); container.innerHTML = ''; const root = document.createElement('div'); root.className = 'game-root'; root.innerHTML = `<h2 class="game-header">Astronomía</h2><div class="game-canvas">Contenido del juego de Astronomía</div>`; container.appendChild(root) }
