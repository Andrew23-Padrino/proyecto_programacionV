...
async function attachGameLinkGuards() {
  try {
    ...
    const anchors = Array.from(document.querySelectorAll('#cursos a'))
      .filter(a => a.getAttribute('href') && a.getAttribute('href').includes('src/pages') && a.getAttribute('href').includes('juego'));
    ...
    anchors.forEach(a => {
      ...
      const isMemoria = href.includes('/src/pages/memoria/') || href.includes('juego_memoria');
