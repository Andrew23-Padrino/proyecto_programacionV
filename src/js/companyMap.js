...
(function(){
  const COMPANY = {
    name: 'Nova Ciencia (Centro Comercial La Boyera)',
  ...
  function initMapsApi(key){
    ...
    window.__nc_init_map = function(){
      try{
        ...
        // add action button to search/mark Nova Ciencia in the iframe fallback
        const action = document.createElement('div');
        action.style.marginTop = '6px';
        action.style.fontSize = '13px';
        action.innerHTML = `<button id="nc-mark-nova" style="padding:6px 10px;border-radius:6px;border:1px solid #cbd5e1;background:#fff;cursor:pointer">Marcar Nova Ciencia</button>`;
        container.appendChild(action);
        const btn = container.querySelector('#nc-mark-nova');
        if (btn) btn.addEventListener('click', ()=>{
          try{
            const q = encodeURIComponent('Nova Ciencia Caracas');
            ...
            if (ifr) { ifr.src = src2; }
              ...
              // add a small control/button to mark "Nova Ciencia" on the map
              try{
                const controlDiv = document.createElement('div');
                controlDiv.style.margin = '8px';
                const btn = document.createElement('button');
                btn.textContent = 'Marcar Nova Ciencia';
                ...
                btn.addEventListener('click', ()=>{
                  try{
                    const query = 'Nova Ciencia, Caracas, Venezuela';
                    geocoder.geocode({ address: query }, (results, status) => {
                      if (status === 'OK' && results && results[0]){
                        ...
                        novaMarker = new google.maps.Marker({ position: loc, map, title: 'Nova Ciencia' });
                        if (novaInfo) novaInfo.close();
                        novaInfo = new google.maps.InfoWindow({ content: `<div style="font-weight:700">Nova Ciencia</div><div style="font-size:13px">${results[0].formatted_address}</div><div style="margin-top:6px"><a href=\"https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(results[0].formatted_address)}\" target=\"_blank\">Ver en Google Maps</a></div>` });
                        novaInfo.open(map, novaMarker);
                      } else {
                        console.warn('Geocode failed for Nova Ciencia', status);
                        alert('No se pudo localizar "Nova Ciencia" con la geocodificación.');
                      }
                    });
